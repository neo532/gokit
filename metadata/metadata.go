...
type clientResponceMetadataKey struct{}

// NewClientResponseContext creates a new context with client responase md attached.
func NewClientResponseContext(ctx context.Context, md Metadata) context.Context {
	return context.WithValue(ctx, clientResponceMetadataKey{}, md)
}

// FromClientResponseContext returns the client response metadata in ctx if it exists.
func FromClientResponseContext(ctx context.Context) (Metadata, bool) {
	md, ok := ctx.Value(clientResponceMetadataKey{}).(Metadata)
	return md, ok
}
